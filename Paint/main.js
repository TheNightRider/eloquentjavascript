...
controls.brushSize = function (cx) {
    var select = elt("select");
    var sizes = [1, 2, 3, 5, 8, 12, 25, 35, 50, 100];
    ...
    function update() {
        try{
            ...
            if (e instanceof SecurityError)
                link.href = "javascript:alert(" + JSON.stringify("Cant save: " + e.toString()) + ")";
            else
                throw e;
        }
    }
...
tools.Rectangle = function (event, cx) {
    var startingPos = relativePos(event, cx.canvas);
    var pageStart = {x: event.pageX, y: event.pageY};

    var trackingRect = elt("div");
    trackingRect.style.position = "absolute";
    trackingRect.style.background = cx.fillStyle;
    document.body.appendChild(trackingRect);

    trackDrag(function (event) {
        var currentPos = {x: event.pageX, y: event.pageY};
        var rect = rectangleFrom(pageStart, currentPos);
        trackingRect.style.left = rect.left + "px";
        trackingRect.style.top = rect.top + "px";
        trackingRect.style.width = rect.width + "px";
        trackingRect.style.height = rect.height + "px";
    }, function (event) {
        var rect = rectangleFrom(startingPos, relativePos(event, cx.canvas));
        cx.fillRect(rect.left, rect.top, rect.width, rect.height);
        document.body.removeChild(trackingRect);
    });
...
function colorAt(cx, pos) {
    var pixel = cx.getImageData(pos.x, pos.y, 1, 1).data;
